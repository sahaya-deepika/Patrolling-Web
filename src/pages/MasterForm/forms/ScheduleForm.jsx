...
// // import { getSchedules, createSchedule, deleteSchedule, updateSchedule, getLocations, getUsers, getTrips } from '../../../api'
...
// //   }
// //   const [zones, setZones] = useState(['Zone A','Zone B','Zone C'])
// //   const [employees, setEmployees] = useState(['Karthick','Arjun','Priya'])
// //   const [tripTypes, setTripTypes] = useState(['1','2','3'])
// //   const [trips, setTrips] = useState(['1','2','3'])

// //   useEffect(() => {
// //     getSchedules().then(setSaved).catch(() => {})
// //     getLocations().then((locs) => {
// //       const z = [...new Set(locs.map((l) => l.zone).filter(Boolean))].sort()
// //       setZones(z.length > 0 ? z : ['Zone A', 'Zone B', 'Zone C'])
// //     }).catch(() => { setZones(['Zone A', 'Zone B', 'Zone C']) })
// //     getUsers().then((users) => {
// //       const u = [...new Set(users.map((u) => u.userName).filter(Boolean))].sort()
// //       setEmployees(u.length > 0 ? u : ['Karthick', 'Arjun', 'Priya'])
// //     }).catch(() => { setEmployees(['Karthick', 'Arjun', 'Priya']) })
// //     getTrips().then((tripsList) => {
// //       const types = [...new Set(tripsList.map((t) => t.tripType).filter(Boolean))].sort()
// //       const names = [...new Set(tripsList.map((t) => t.tripName).filter(Boolean))].sort()
// //       setTripTypes(types.length > 0 ? types : ["1","2","3"])
// //       setTrips(names.length > 0 ? names : ["1","2","3"])
// //     }).catch(() => { setTripTypes(["1","2","3"]); setTrips(["1","2","3"]) })
...
//       setTripTypes(t.length > 0 ? t : ['1', '2', '3'])
//     }).catch(() => { setTripTypes(['1', '2', '3']) })

//     // Trip options — from trips created in TripForm
//     getTrips().then((tripsList) => {
//       const names = [...new Set(tripsList.map((t) => t.tripName).filter(Boolean))].sort()
//       setTrips(names.length > 0 ? names : ['1', '2', '3'])
//     }).catch(() => { setTrips(['1', '2', '3']) })
//   }, [])
