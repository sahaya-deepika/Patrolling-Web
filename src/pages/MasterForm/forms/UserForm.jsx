...
export default function UserForm() {
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return
    const handleMastersUpdated = () => refreshMasters()
    window.addEventListener('masters-updated', handleMastersUpdated)
    window.addEventListener('zones-updated', handleMastersUpdated) // backward compat
    return () => {
      window.removeEventListener('masters-updated', handleMastersUpdated)
      window.removeEventListener('zones-updated', handleMastersUpdated)
    }
  }, [])
