...
// const blank = {
//   patrolType: '',
//   questionType: 'Option', // 'Fill' | 'YesNo' | 'Option'
//   question: '',
//   options: ['', ''],
// }
...
//   useEffect(() => {
//     getTasks().then(setSaved).catch(() => {})
//   }, [])

//   const set = (k, v) => setForm((p) => ({ ...p, [k]: v }))

//   const setOption = (i, val) => {
//     const updated = form.options.map((o, idx) => idx === i ? val : o)
//     set('options', updated)
//   }
...
//   const handleClone = (item) => { const { id, ...rest } = item; setForm({ ...blank, ...rest, options: rest.options?.length ? rest.options : blank.options }); setSel(null) }
...
//     try {
//       if (sel) {
//         const updated = await updateTask(sel, payload)
...
//     for (const id of selectedIds) { try { await deleteTask(id) } catch {} }
...
//             onChange={(e) => set('patrolType', e.target.value)}
//             style={{ padding: '10px 14px', border: '1px solid #dadce0', borderRadius: '8px', fontSize: '13px', color: '#202124', outline: 'none', height: '42px', boxSizing: 'border-box', background: '#fff' }}
//           />
...
//                 {selectedIds.length > 0 ? `${selectedIds.length} selected` : 'Saved Task'}
//               </h2>
...
//           ) : (
//             <>
//               {showSaved && <h2 style={{ fontSize: '16px', fontWeight: 500, color: '#202124', margin: 0, lineHeight: '28px' }}>Saved Task</h2>}
...
//             {saved.length === 0 && (
//               <div style={{ textAlign: 'center', color: '#9aa0a6', fontSize: '13px', marginTop: '40px' }}>No tasks saved yet.</div>
//             )}
//           </div>
//         )}
//       </div>
...
import { useState, useEffect, useRef } from 'react'
import { getTasks, createTask, deleteTask, updateTask, getPatrolTypes } from '../../../api'
import { ConfirmSaveModal } from '../components/MasterFormUI'
...
export default function TaskForm() {
  ...
  useEffect(() => {
    getTasks().then(setSaved).catch(() => {})
    // Fetch patrol types from their master source (Others → Patrol Types)
