...
export default function Attendance() {
  ...
  // allData — always from /attendance/all (used for dept options + punctual list)
  const [allData, setAllData]     = useState(EMPTY)
  // attList — may be from /attendance/all or /attendance/department-wise
  const [attList, setAttList]     = useState(EMPTY)
  const [selected, setSelected]   = useState(null)
  const [empDetail, setEmpDetail] = useState(EMPTY)
  const [punct, setPunct]         = useState(EMPTY)
  ...
  // deptMap: { [deptName]: deptId } — built from allData so we can pass dept_id to API
  const deptMap     = useRef({})
  const fetchId     = useRef(0)
  const deptFetchId = useRef(0)
  const dateStr     = useMemo(() => toStr(date), [date])

  // ── 1) Main fetch: triggered by date / unit / shift changes ──────────────────
  useEffect(() => {
    const id = ++fetchId.current
    const filters = { unit, shift, date: dateStr }
    ...
    // /attendance/all — drives allData, attList (when no dept filter), and punct
    fetchAttendanceList(filters).then(d => {
      if (id !== fetchId.current) return
      // Build deptName → deptId map for dept-wise API calls
      const map = {}
      ;(d.employees || []).forEach(e => {
        if (e.deptId != null && e.department) map[e.department] = e.deptId
      })
      deptMap.current = map
      setAllData({ data: d, loading: false, error: null })
      setAttList({ data: d, loading: false, error: null })
    }).catch(fail(setAllData))

    // /attendance/all → derive punctual list (sorted by earliest login)
    fetchPunctualEmployees(filters).then(safe(setPunct)).catch(fail(setPunct))
  }, [unit, shift, dateStr])

  // ── 2) Dept filter: calls /attendance/department-wise when a dept is chosen ──
  useEffect(() => {
    if (deptFilter === 'All') {
      // Restore full list from allData (no extra API call needed)
      setAttList(allData)
      return
    }

    const deptId = deptMap.current[deptFilter]
    if (!deptId) {
      // Fallback: dept id unknown — filter the full list client-side
      return
    }

    const id = ++deptFetchId.current
    setAttList(EMPTY)
    setSelected(null)

    fetchAttendanceList({ unit, shift, date: dateStr, department_id: deptId })
      .then(d => {
        if (id !== deptFetchId.current) return
        setAttList({ data: d, loading: false, error: null })
      })
      .catch(e => {
        if (id !== deptFetchId.current) return
        setAttList({ data: null, loading: false, error: e.message })
      })
  ...
  // Fetch employee detail on selection
  useEffect(() => {
    if (!selected) return
    setEmpDetail(EMPTY)
    setExpandedSet(new Set())
  ...
  // Status — reads directly from API fields, no frontend recalculation
  const getEmployeeStatus = useCallback((emp) => {
    if (emp.status === 'absent') return 'absent'
    return emp.isLate ? 'late' : 'on-time'
  }, [])

  // Displayed employee list (from attList — may be dept-filtered by API)
  const employees = attList.data?.employees || []

  // Dept options always from allData so the dropdown never shrinks when filtered
  const deptOptions = useMemo(() => {
    const allEmps = allData.data?.employees || []
    const depts = [...new Set(allEmps.map(e => e.department).filter(Boolean))].sort()
    return ['All', ...depts]
  }, [allData.data])

  // Only status/ontime/late filter applied client-side; dept is handled by the API
  const filteredEmployees = employees.filter(emp => {
    const status = getEmployeeStatus(emp)
    // Client-side fallback dept filter when deptMap had no id
    if (deptFilter !== 'All' && !deptMap.current[deptFilter] && emp.department !== deptFilter) return false
    if (mainFilter === 'absent') return status === 'absent'
    if (detailFilter === 'all') return status !== 'absent'
    if (detailFilter === 'on-time') return status === 'on-time'
    if (detailFilter === 'late') return status === 'late'
