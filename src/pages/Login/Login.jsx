...
//   // Direct check on all common token/api-key names (top-level)
//   const TOKEN_KEYS = [
//     'api_key', 'apiKey',                                         // ← your backend
...
//     // Fallback: any key containing "token" or "key" with a string value
...
//   // Top-level check first (your backend returns name & user_id at root)
...
// // ── Build Authorization Header based on backend type ──
// // If the stored credential is a plain API key (not JWT), send as X-API-Key.
// // If it's a JWT, send as Bearer. Adjust to match your backend's expectation.
// export function getAuthHeader() {
//   const token = auth.get()
//   if (!token) return {}
//   return auth.isJwt(token)
//     ? { Authorization: `Bearer ${token}` }
//     : { 'X-API-Key': token }          // ← plain api_key from your backend
// }
...
//     decoded: auth.decode(token),      // null if plain API key — safe to use
//     user: resolveUser(response, user_id),
//   }
// }

// async function apiChangePassword(user_id, currentPw, newPw) {
//   throw new Error('Change password is not implemented for the backend yet.')
// }
...
//       // Save api_key or JWT — auth.get() works for both
//       auth.save(data.token)

//       // Pass full data (includes isJwt, decoded, user) to parent
//       onLoginSuccess?.(data)
//     } catch (err) {
//       auth.clear()
...
//     setLoading(true)
//     try {
//       await apiChangePassword(user_id.trim(), currentPw, newPw)
...
//             />
//             <button className="login-btn" type="submit" disabled={loading}>
//               {loading ? <><span className="btn-spinner" />Updating...</> : 'Change Password'}
//             </button>
//           </form>
//         )}
...
import { useState, useCallback } from 'react'
import { login, getAuthHeader } from '../../api'
import './Login.css'
